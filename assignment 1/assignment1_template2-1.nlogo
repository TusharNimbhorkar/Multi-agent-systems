...
; --- Setup ---
to setup
  clear-all ;it wasnt here before , added
  set time 0
  reset-timer
  setup-patches
  setup-vacuums
...
to go
  ...
  update-beliefs
  update-desires
  if desire = "Going to sleep now."
  [stop]
  if desire = "Need to empty bag and then sleep"
  [ set bag bag_size
    update-intentions
    execute-actions
    ;Set desire "Going to sleep now."
    stop ]
  update-intentions
  execute-actions
  ]
...
to setup-patches
  ...
  ; set total dirty
  set total_dirty  round ( dirt_pct / 100 * width * height )
  ask n-of total_dirty patches [set pcolor brown]

end

to setup-bins
  create-bins 1 [set color black ]
...
to setup-vacuums
  ...
  ; create vacs
  create-vacuums 1 [set color white ]
  ask vacuums [ set desire ( count patches with [ pcolor = brown ] )
  set bag 0]
  ; ask something about setiing the desire

  ;let tmp patches with [pcolor = brown]
  ;ask vacuums [ set beliefs tmp ]
...
to update-desires
  ...
  ask vacuums [
    let tmp count patches with [ pcolor = brown ]
    ifelse(tmp < 1)
    [
      ifelse(bag < 1)
      [set desire ( "Going to sleep now." ) ]
      [set desire ("Need to empty bag and then sleep")]
    ]
    [
      ifelse( bag > 4)
      [
        set desire ( "Going to Garbage can" )
      ]
      [
        ifelse( tmp > 1)
        [set desire ( word "Need to clean " tmp " amount of patches" ) ]
        [set desire ( "Only one more patch left")]
      ]
    ]
  ]
  ;stopping condition? related to desire?
...
to update-intentions
  ...
  ask vacuums [
    ifelse( bag > bag_size - 1 )
    [set intention ( min-one-of bins [distance myself] )]
    [set intention (min-one-of beliefs [distance myself])]
    ;set intention ( last beliefs )
  ]
...
to execute-actions
  ...
  ;ask vacuums (towardsxy (patches with [ pcolor = brown ] ) )

  ask vacuums [
    ;ifelse ( bag
    ifelse ( round ( distance intention ) = 0 )
    [ ifelse(round ( distance one-of bins ) = 0)
      [set bag 0
      set pcolor green]; not sure about this
      [set pcolor green
       set bag bag + 1
